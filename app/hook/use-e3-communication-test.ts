...
export function useE3CommunicationTest() {
    ...
    for (let port of ports) {
      if (!port.readable && !port.writable && !identified.find(el => el.port === port)) {
        ...
        try {
          ...
          let imei: Response, iccid: Response, et: Response
  ...
  const getDeviceConfig = async (port: ISerialPort) => {
    if (!port.readable && !port.writable) {
      ...
      try {
        ...
        let attempts = 0
        const max_retries = 3
        let check: Response, cxip: Response, dns: Response, et: Response, status: Response;
  ...
  const sendCommandWithRetries = async (port: ISerialPort, command: string, maxRetries = 3) => {
    let attempts = 0;
  ...
  const sendCommandsToPorts = async (ports: ISerialPort[], commands: string[]) => {
    for (let port of ports) {
      try {
        ...
        await closePort(port)
        await sendCommandsToPorts([port], commands)
      }
    }
  }
  ...
  return { identified, requestPort, ports, sendCommandsToPorts, commands, getDeviceConfig, config }
}
